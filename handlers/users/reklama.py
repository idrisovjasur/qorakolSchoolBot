...
@dp.message_handler(text='/reklama_video',user_id=ADMINS[1])
...
@dp.message_handler(text = '/add_teacher',user_id = ADMINS[1])
@dp.message_handler(text = '/add_teacher',user_id = ADMINS[0])
async def teacher_def(message:Message,state:FSMContext):
    await message.answer('Admin qanday fandan ustoz kiritmoqchisiz?',reply_markup=how_subject)
    await state.set_state('how_subject')
...
async def send_file_danne(message:Message,state:FSMContext):
        document = Document()
